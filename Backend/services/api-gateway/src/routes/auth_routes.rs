...
async fn login_handler(
    ...
    let mut client = match state.grpc_clients.read().await.auth_client().await {
        Ok(c) => c,
        Err(e) => return (StatusCode::SERVICE_UNAVAILABLE, Json(json!({
            "success": false, "code": 503, "message": format!("认证服务不可用: {e}")
        }))).into_response(),
    };
    ...
    match client.login(username.clone(), password).await {
            ...
            let refresh_token = match state.jwt_service.generate_refresh_token(resp.user_id) {
                ...
                Err(e) => {
                    ...
                    return (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({
                        "success": false, "code": 500, "message": format!("生成刷新令牌失败: {}", e)
                    }))).into_response();
                }
            };
        ...
        Err(e) => {
            ...
            (StatusCode::UNAUTHORIZED, Json(json!({
                "success": false, "code": 401, "message": format!("登录失败: {e}")
            }))).into_response()
        }
    }
}
...
async fn register_handler(
    ...
    match client.register(
        ...
        Ok(resp) => (StatusCode::OK, Json(json!({
            ...
            "data": { "token": resp.token, "user_id": resp.user_id, "username": resp.username }
        }))).into_response(),
        Err(e) => (StatusCode::BAD_REQUEST, Json(json!({
            "success": false, "code": 400, "message": format!("注册失败: {e}")
        }))).into_response(),
    }
}
...
async fn get_user_info_handler(
    ...
    let mut client = match state.grpc_clients.read().await.user_client().await {
        Ok(c) => c,
        Err(e) => return (StatusCode::SERVICE_UNAVAILABLE, Json(json!({
            "success": false, "code": 503, "message": format!("用户服务不可用: {e}")
        }))).into_response(),
    };
    ...
    match client.get_user(claims.sub).await {
        ...
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({
            "success": false, "code": 500, "message": format!("获取用户信息失败: {e}")
        }))).into_response(),
    }
}
...
async fn refresh_token_handler(
    ...
    match client.refresh_token(refresh_token).await {
        Ok(resp) => (StatusCode::OK, Json(json!({
            "success": true, "code": 200,
            "data": { "access_token": resp.token, "user_id": resp.user_id, "username": resp.username, "role": resp.role }
        }))).into_response(),
        Err(e) => (StatusCode::UNAUTHORIZED, Json(json!({
            "success": false, "code": 401, "message": format!("Token刷新失败: {e}")
        }))).into_response(),
    }
}
...
async fn update_profile_handler(
    ...
    match client.update_user(
        ...
        Ok(resp) => (StatusCode::OK, Json(json!({
            "success": true, "code": 200,
            "data": { "id": resp.id, "username": resp.username, "nickname": resp.nickname }
        }))).into_response(),
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({
            "success": false, "code": 500, "message": format!("更新资料失败: {e}")
        }))).into_response(),
    }
}
...
async fn change_password_handler(
        ...
        || !new_password.chars().any(|c| c.is_ascii_digit())
    {
        return (StatusCode::BAD_REQUEST, Json(json!({
            "success": false, "code": 400, "message": "新密码必须不少于 8 位且同时包含字母和数字"
        }))).into_response();
    }
    ...
    if !auth_ok {
        return (StatusCode::UNAUTHORIZED, Json(json!({
            "success": false, "code": 401, "message": "原密码验证失败"
        }))).into_response();
    }
    ...
    match update_result {
        ...
        Err(e) => (StatusCode::INTERNAL_SERVER_ERROR, Json(json!({
            "success": false, "code": 500, "message": format!("密码修改失败: {e}")
        }))).into_response(),
    }
}
