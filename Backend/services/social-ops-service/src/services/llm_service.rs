...
impl LlmService {
    ...
    pub async fn list_providers(&self) -> Result<Vec<Value>, sqlx::Error> {
        sqlx::query!(
            r#"SELECT id, provider_name, api_endpoint, model_name, api_key_enc, is_active
    ...
    pub async fn add_provider(&self, name: &str, endpoint: &str, api_key: &str, model: &str) -> Result<Value, sqlx::Error> {
        sqlx::query!(
            r#"INSERT INTO socialops.llm_providers (provider_name, api_endpoint, api_key_enc, model_name)
             VALUES ($1, $2, $3, $4)
             RETURNING id, provider_name, api_endpoint, model_name, is_active"#,
            name,
            endpoint,
            api_key,
            model,
    ...
    pub async fn delete_provider(&self, id: Uuid) -> Result<bool, sqlx::Error> {
        let r = sqlx::query!("DELETE FROM socialops.llm_providers WHERE id = $1", id)
