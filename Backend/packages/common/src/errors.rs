...
impl IntoResponse for AppError {
    fn into_response(self) -> Response {
        let (status, error_code, error_message) = match &self {
            ...
            // CTP 锁平台服务
            Self::CtpDeviceConnection(_) => (
                StatusCode::BAD_GATEWAY,
                44001,
                self.to_string(),
            ),
            Self::CtpDeviceCommand(_) => (
                StatusCode::BAD_GATEWAY,
                44002,
                self.to_string(),
            ),
            Self::CtpDeviceOffline => (
                StatusCode::SERVICE_UNAVAILABLE,
                44003,
                self.to_string(),
            ),
            Self::CtpDeviceNotFound(_) => (
                StatusCode::NOT_FOUND,
                44004,
                self.to_string(),
            ),
            Self::CtpInvalidParams => (
                StatusCode::BAD_REQUEST,
                44005,
                self.to_string(),
            ),
            ...
            // XLT 信路通服务
            Self::XltMqttConnection(_) => (
                StatusCode::BAD_GATEWAY,
                48001,
                self.to_string(),
            ),
            Self::XltMqttPublish(_) => (
                StatusCode::BAD_GATEWAY,
                48002,
                self.to_string(),
            ),
            Self::XltVehicleNotFound => (
                StatusCode::NOT_FOUND,
                48003,
                self.to_string(),
            ),
            Self::XltInvalidParams => (
                StatusCode::BAD_REQUEST,
                48004,
                self.to_string(),
            ),
            ...
            // 支付服务
            Self::PayOrderNotFound => (
                StatusCode::NOT_FOUND,
                49001,
                self.to_string(),
            ),
            Self::PayInvalidParams => (
                StatusCode::BAD_REQUEST,
                49002,
                self.to_string(),
            ),
